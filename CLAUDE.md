...
**Commitments — history section** (bottom card on the page, `renderCommitments`, `js/pages/commitments.js`): three tabs — **Week / Month / Year**, all derived purely from `goalLogs` already held in memory (no extra fetch). State: `commitPreviewTab` (`'week'|'month'|'year'`), `commitViewWeekStart`, `commitViewMonth`, `commitHeatmapYear` — each tab remembers its own navigation position independently. None of the three allow navigating into the future (no compliance data can exist there); all allow navigating arbitrarily far into the past.
- **Day was removed as a standalone tab** (2026-09-18) — it only ever showed one date at a time with no calendar context around it, so as a first-class tab it was low-value. Replaced by **click-to-drill on a Month or Year cell** (see "Commitments — day-detail drill-down" below), which shows the same per-commitment breakdown one click away from the calendar/heatmap it belongs to.
- **Week** — the old "weekly strip" (7 circles, Mon–Sun), navigable to any past week via `getMondayOf()` / `getWeekDays()`.
- **Month** — a compliance calendar grid, navigable to any past month. Each past/today cell is clickable (opens the day-detail panel below the grid) and tinted with a **continuous** heat gradient (`commitHeatTint()` — blends `--danger`→`--accent` by that day's %, then mixes to 55% opacity — replacing the old 3-bucket low/mid/high scheme for a smoother look) instead of a flat 3-tier color. A `commit-month-stats` line above the grid shows that month's average % and perfect-day count.
- **Year** — a GitHub-style contribution heatmap (`buildCommitYearHeatmapData()` / `renderCommitYearHeatmap()`, reuses the shared `.proj-heatmap`/`.heatmap-cell` CSS) — one cell per day, tinted with the same continuous `commitHeatTint()` gradient as Month, clickable to open the day-detail panel. The header line now also reports perfect-day and active-day counts alongside the average %, and a Less→More legend sits under the grid. Distinct from Home's Activity heatmap below: this one measures pass/fail compliance, Home's measures raw activity volume.

**Commitments — day-detail drill-down** (`commitDayDetailHtml()`, `js/pages/commitments.js`, state: `commitViewDay`, nullable — an iso date or `null`): clicking any clickable Month/Year cell (`data-commit-day-select`) toggles `commitViewDay` to that date and renders a detail panel directly below the grid/heatmap it was clicked in (click the same cell again, or the panel's `×`, to close). Cleared automatically on tab switch and on any Month/Year prev/next/"today" navigation, since it belongs to whichever calendar view it was opened from. Shows every commitment for that date grouped by category (`getGoalCategories()`/`categoryItems()`, matching the main page's grouping) with its checked/unchecked state, `count/target` for counter commitments, its `reminder_time` cue badge if set, and — new, and the reason this replaced the old Day tab instead of just relocating it — the actual `goal_logs.completed_at` time if it was checked that day, so the panel shows not just whether a commitment was done but roughly when.
