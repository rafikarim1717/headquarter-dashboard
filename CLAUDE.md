...
### `custom_stations`
Backs the ambient music widget's user-added YouTube stations (`window.__HQ_TWEAKS.customStations`, not `state` — see "Ambient Music Widget" above). In both schema files (`schema_fix.sql` section 22). Previously `localStorage`-only (device-scoped); moved here so stations follow the user across devices.
| Column | Type | Notes |
|---|---|---|
| `id` | uuid PK | |
| `user_id` | uuid FK | → `auth.users` |
| `name` | text | Default `''`. Station label shown in the picker |
| `url` | text | Default `''`. A YouTube watch/live/embed/shorts/`youtu.be` link — `extractYouTubeId()` parses it client-side |
| `created_at` | timestamptz | Load order (`order('created_at')`) |

Operations: `insert` (on `+ Add station`, a blank row — awaited so the row has a real id before it's added to state), `update name/url` (debounced 1000ms per keystroke), `delete` (on 🗑, immediate, no confirm). Not seeded for new users.
...
- **Kanji instead of nav icons** (sidebar only, `.nav-kanji`): 今日 Today · 予定 Schedule · 習慣 Commitments · 計画 Projects · 覚書 Notes · 概要 Overview · 収入 Income · 支出 Spending · 借金 Debts. The kanji sits in the old icon slot (idle `--text-faint`, active page `--shu`); collapsed (60px) the sidebar is a column of kanji with the hanko on top — the collapse toggle (a sidebar-panel icon, not an arrow) is hidden while collapsed, and clicking the hanko expands the sidebar again. The **Life / Finance section labels and the mobile bottom nav stay English/icon-only** on purpose (the user found kanji there too busy). Sign out keeps its SVG icon.
...
**Custom station persistence is the one exception to Tweaks' normal (in-memory-only) behavior**: every other Tweak resets to its hardcoded `index.html` default on refresh (see `window.__HQ_TWEAKS`), but `customStations` (a plain array, any length) is backed by its own Supabase table, `custom_stations` (schema_fix.sql section 22) — one row per station, `{ id, name, url }`. `loadFromSupabase()` populates `window.__HQ_TWEAKS.customStations` at login (same as it does `profile.name`), so stations follow the user across devices instead of being stuck on one browser. `addCustomStation()` awaits an insert (to get a real row id) before pushing the new blank row into state; typing into a row's name/url field debounces 1000ms before writing (`setCustomStationField()`, mirrors Notes' autosave) so fast typing doesn't fire a write per keystroke; `removeCustomStation()` deletes by id. (The Tweaks "Your name" field is the other DB-backed one — it updates `profiles.name`.)
...
### Data Fetch
- All data fetched in parallel via `Promise.all()` in `loadFromSupabase()` (profiles, schedule_events, goals, goal_logs, projects, project_tasks, income_entries, spending_entries, debts, notes, today_focus_items, custom_stations).
...
13. **`custom_stations` table** — both schema files (`schema_fix.sql` section 22). Needed for the ambient music widget's custom YouTube stations; without it, custom stations load empty every login and `+ Add station` fails outright (they used to survive via `localStorage` alone — that fallback is gone now that they're per-user).
