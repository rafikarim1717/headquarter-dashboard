...
**Commitments — history section** (bottom card on the page, `renderCommitments`, `js/pages/commitments.js`): two tabs — **Grid / Year**. State: `commitPreviewTab` (`'grid'|'year'`) — any legacy `'day'|'week'|'month'` value from before 2026-09-18 falls back to `'grid'`.
- **Grid** (`renderCommitHabitGrid()`) — a habit-tracker **table**: rows = every commitment, columns = days, so a glance shows both per-commitment patterns (read a row) and same-day comparisons (read a column) at once — replaced the old separate Week strip + Month calendar tabs (2026-09-18), which only ever showed an aggregate %, never the per-commitment breakdown, without drilling into a specific day. `commitGridRange` (`'week'|'month'`, a toggle inside the Grid tab, not a separate top-level tab) sets the column count: `'week'` uses `commitViewWeekStart`/`getWeekDays()` (7 columns, Mon–Sun letters + date), `'month'` uses `commitViewMonth` (up to 31 columns, date numbers only — no weekday letters, too cramped). Both reuse the exact same prev/next/"today" nav bindings (`data-commit-week-nav`/`data-commit-month-nav` etc.) the old Week/Month tabs used. Rows are ordered by `getGoalCategories()`/`categoryItems()` (category-grouped, no inline label — the point is a fast per-row/per-column scan, not a second copy of the category cards above) and each shows its 🔥 streak next to the name. A `target_count === 1` cell shows ✓/· (checked/not); a counter commitment (`target_count > 1`) shows the raw `count` number instead, tinted once it reaches `target_count`. A `Daily %` footer row (tinted via `commitHeatTint()`, see below) keeps the old aggregate-compliance-per-day info alongside the per-commitment rows. The name column is `position: sticky; left: 0` (`.habit-table-namecol`) so it stays visible while `.habit-table-wrap` scrolls horizontally under Month's wider range.
- **Year** — a GitHub-style contribution heatmap (`buildCommitYearHeatmapData()` / `renderCommitYearHeatmap()`, reuses the shared `.proj-heatmap`/`.heatmap-cell` CSS) — one cell per day, tinted with a **continuous** heat gradient (`commitHeatTint()` — blends `--danger`→`--accent` by that day's %, then mixes to 55% opacity, rather than a flat 3-bucket low/mid/high scheme), clickable to open the day-detail panel (see below). The header line reports average/perfect-day/active-day counts for the year, and a Less→More legend sits under the grid. Distinct from Home's Activity heatmap below: this one measures pass/fail compliance, Home's measures raw activity volume.
- Naming note: the habit-tracker table's CSS/markup is prefixed `habit-table-*`, not `habit-grid-*` — an unrelated, unused `.habit-grid` class already existed in `css/styles.css` (dead CSS left over from the pre-Commitments Habits page) and silently broke the table's layout (`display: grid` collided with the table's own layout) the first time this was built with that name.

**Commitments — day-detail drill-down** (`commitDayDetailHtml()`, `js/pages/commitments.js`, state: `commitViewDay`, nullable — an iso date or `null`): clicking a clickable Year cell (`data-commit-day-select`) toggles `commitViewDay` to that date and renders a detail panel directly below the heatmap (click the same cell again, or the panel's `×`, to close). Cleared automatically on tab switch and on Year prev/next navigation, since it belongs to whichever year view it was opened from. Year is the only tab that still needs this — Grid already shows every commitment's per-day state inline, so a separate drill-down would be redundant there. Shows every commitment for that date grouped by category (`getGoalCategories()`/`categoryItems()`, matching the main page's grouping) with its checked/unchecked state, `count/target` for counter commitments, its `reminder_time` cue badge if set, and the actual `goal_logs.completed_at` time if it was checked that day, so the panel shows not just whether a commitment was done but roughly when.
